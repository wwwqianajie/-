...
public class HbaseConnector {
    ...
    public List<String> getDataFromTable(String tableName) throws IOException {
        ...
        for (Result result : scanner) {
            String value = Bytes.toString(result.getValue(Bytes.toBytes("rank_id"), Bytes.toBytes("price_range")));
            String value1 = Bytes.toString(result.getValue(Bytes.toBytes("rank_id"), Bytes.toBytes("count")));
            results.add(value);
            results.add(value1);
        }
